...
class OpenGLGlyphs:
    ...
    def _draw_scene(self):
        ...
        ix = bg_image.size[0]
        iy = bg_image.size[1]
        #bg_image = np.fromstring(bg_image.tostring(), np.uint8)
        #bg_image=base64.b64encode(bg_image)
        bg_image = bg_image.tobytes("raw", "BGRX", 0, -1)
        print("bg image data type is")
        print(type(bg_image))
    ...
    def _handle_glyphs(self, image):
        ...
        for glyph in glyphs:
            ...
            if glyph_name == SHAPE_MAP:
                print("before gl_list")
                glCallList(self.map.gl_list)
                print("after gl_list")
